...
Deferred operation parameter schemas and descriptions must be returned in the `search_tools` result, not inserted into `systemPrompt` or the `tools` array. Internal output schemas stay in the registry for canonical-result validation and are not part of model-visible manifests. `search_call` validates the requested operation against the persisted/folded active capability state, validates its arguments and canonical result, forwards `exec.signal`, and preserves the operation's model rendering. A call before disclosure must fail closed.
...
Every parameter schema and internal operation output schema is capped at 16 KiB, each complete slim operation manifest is capped at 32 KiB, capability notices are capped at 64 KiB, and the complete canonical and model-text `search_tools` JSON values are capped at 16 KiB. The source-produced `search_sources` notice has a stricter 4 KiB cap. Model-visible manifests omit output schemas; the registry's internal snapshots and validation bounds remain authoritative. These are complete-value bounds, not truncation points: an over-limit manifest or compact `search_tools` value fails closed before it can be disclosed. A deferred call therefore has one canonical JSON result regardless of whether it came from Native `search_call({ operation, arguments })` or a Code SDK binding to `tools.search_call(...)`.

## Integrated gateway contract

The implemented operation manifest carries each deferred operation's exact `parameters` plus the canonical `search_call` route, while the output schema remains an internal registry snapshot. `search_tools` returns the same immutable definitions in both discovery modes:
